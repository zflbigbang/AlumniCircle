...
var (
	...
	cacheAlumniCircleLikeLikeRecordIdPrefix               = "cache:AlumniCircleLike:likeRecord:id:"
	cacheAlumniCircleLikeLikeRecordBizIdObjIdUserIdPrefix = "cache:AlumniCircleLike:likeRecord:bizId:objId:userId:"
)
...
func (m *defaultLikeRecordModel) Delete(ctx context.Context, id int64) error {
	...
	AlumniCircleLikeLikeRecordBizIdObjIdUserIdKey := fmt.Sprintf("%s%v:%v:%v", cacheAlumniCircleLikeLikeRecordBizIdObjIdUserIdPrefix, data.BizId, data.ObjId, data.UserId)
	AlumniCircleLikeLikeRecordIdKey := fmt.Sprintf("%s%v", cacheAlumniCircleLikeLikeRecordIdPrefix, id)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, AlumniCircleLikeLikeRecordBizIdObjIdUserIdKey, AlumniCircleLikeLikeRecordIdKey)
	return err
}
...
func (m *defaultLikeRecordModel) FindOne(ctx context.Context, id int64) (*LikeRecord, error) {
	AlumniCircleLikeLikeRecordIdKey := fmt.Sprintf("%s%v", cacheAlumniCircleLikeLikeRecordIdPrefix, id)
	var resp LikeRecord
	err := m.QueryRowCtx(ctx, &resp, AlumniCircleLikeLikeRecordIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultLikeRecordModel) FindOneByBizIdObjIdUserId(ctx context.Context, bizId string, objId int64, userId int64) (*LikeRecord, error) {
	AlumniCircleLikeLikeRecordBizIdObjIdUserIdKey := fmt.Sprintf("%s%v:%v:%v", cacheAlumniCircleLikeLikeRecordBizIdObjIdUserIdPrefix, bizId, objId, userId)
	var resp LikeRecord
	err := m.QueryRowIndexCtx(ctx, &resp, AlumniCircleLikeLikeRecordBizIdObjIdUserIdKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultLikeRecordModel) Insert(ctx context.Context, data *LikeRecord) (sql.Result, error) {
	AlumniCircleLikeLikeRecordBizIdObjIdUserIdKey := fmt.Sprintf("%s%v:%v:%v", cacheAlumniCircleLikeLikeRecordBizIdObjIdUserIdPrefix, data.BizId, data.ObjId, data.UserId)
	AlumniCircleLikeLikeRecordIdKey := fmt.Sprintf("%s%v", cacheAlumniCircleLikeLikeRecordIdPrefix, data.Id)
...
func (m *defaultLikeRecordModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheAlumniCircleLikeLikeRecordIdPrefix, primary)
}
